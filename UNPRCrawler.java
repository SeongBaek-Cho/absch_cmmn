...
@Component
public class UNPRCrawler extends RequestCrawler {

	
	private UNPRCrawler() {}
	
	public UNPRCrawler(String url) {
		try {
			if (!url.isEmpty()) {
...
//				System.out.println(element);
				...
				Element date;
				try {
					title = element.getElementsByTag("p").get(0);
					href = element.getElementsByTag("a").get(0);
					date = element.getElementsByTag("a").get(0);
				} catch (Exception d) {
					continue;
				}
				...
				map.put(super.date, date.attr("href").substring(6, 16));
				System.out.println(map.get(super.date));
				array.add(map);
			}
			return array;
		}
